...
putnam_ts <- read.csv("data1.csv", header = TRUE)
...
lognik.arma.garch.norm1111 <- ugarchfit(data = global, spec = arma.garch.norm1111)
...
putnam.garch.norm1111 <- ugarchfit(data = fit, spec = arma.garch.norm1111)
show(putnam.garch.norm1111)
res1111 = residuals(putnam.garch.norm1111, standardize=FALSE)
res1111.std = residuals(putnam.garch.norm1111, standardize=TRUE)
par(mfrow=c(2,3))
plot(res1111)
acf(res1111)
acf(res1111^2)
...
library(fBasics)
library(fGarch)
Gfit = garchFit(formula = ~garch(1,1),data = fit, trace=FALSE)
