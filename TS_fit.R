...
putnam_ts <- read.csv("data2.csv", header = TRUE)
...
qqnorm(residuals(us_fit))
qqline(residuals(us_fit))
#------
fitt <- function(re)
{
p=0
...
------loop run-----
putnam_ts <- read.csv("data2.csv", header = TRUE)
var = vars::VAR(putnam_ts,p=1,type="const")
pr = predict(var,n.ahead=5,ci=0.95)
r_list = c()
name_list = c("Global.Term","US.Term","US.TIPS.Excess.Ret","Global.Linkers.Excess.Ret","US.TIPS.Breakeven","WGBI.FX","GBI.EM.Term","GBI.EM.FX","EMBIG.Div","IG.Corp","HY.Corp","Leveraged.Loan","US.Convertibles","ABS.Composite","CMBS.Composite")
r_list = lapply(pr$fcst[name_list],'[',,1)
